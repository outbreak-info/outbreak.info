    ...
    <div class="col-sm-12 bg-light d-flex justify-content-center align-items-center my-5" style="min-height: 70vh;">
      <div class="container half-page">
        ...
        <div class="mb-4 border-top border-bottom py-2">
          <p class="mb-1">
            A summary of the major additions and data changes to outbreak.info. Individual changes are tracked on
            <a href="https://github.com/outbreak-info" target="_blank">GitHub</a>.
          </p>
          <div class="d-flex align-items-center justify-content-center flex-wrap">
            Data is updated approximately daily. Last updated:
            <div class="d-flex ml-1">
              <div id="genomics-updated" class="mr-3">
                <router-link :to="{name: 'SituationReports'}" class="">Variants</router-link>
                <small class="text-muted badge bg-grey__lightest mt-1" v-if="lastUpdated && lastUpdated['genomics']">
                  <font-awesome-icon class="mr-1" :icon="['far', 'clock']" /> {{ lastUpdated['genomics'] }} ago
                </small>
              </div>

              <div id="resources-updated" class="mr-3">
                <router-link :to="{name: 'Resource Summary'}" class="">Research Library</router-link>
                <small class="text-muted badge bg-grey__lightest mt-1" v-if="lastUpdated && lastUpdated['resources']">
                  <font-awesome-icon class="mr-1" :icon="['far', 'clock']" /> {{ lastUpdated['resources'] }} ago
                </small>
              </div>

              <div id="epi-updated" class="mr-3">
                <router-link :to="{name: 'Epidemiology'}" class="">Cases & Deaths</router-link>
                <small class="text-muted badge bg-grey__lightest mt-1" v-if="lastUpdated && lastUpdated['epi']">
                  <font-awesome-icon class="mr-1" :icon="['far', 'clock']" /> {{ lastUpdated['epi'] }} ago
                </small>
              </div>
            </div>
