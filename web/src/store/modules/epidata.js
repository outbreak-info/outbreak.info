    ...
    from(csv(store.state.casesUrl))
      .pipe(
        tap(data => {
          ...
          const regionTotals = nestEpiTrace(
            cleanedCases.flatMap(d => d.data),
            "region",
            "region"
          ).filter(
            d => !["Diamond Princess Cruise", "China"].includes(d.locationName)
          );
          const countryTotals = nestEpiTrace(
            cleanedCases.flatMap(d => d.data),
            "country",
            "country"
          );
          const subnationals = cleanedCases.filter(
            d => d.locationType === "sub-national"
          );
          const cleaned = [regionTotals, countryTotals, subnationals].reduce(
