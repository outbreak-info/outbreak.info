    ...
    <div :style="{ height: height + 'px', width: width + 'px' }" id="case-map">
      ...
      >
        ...
        <div v-for="(circle, i) in data" :key="i">
          <l-circle-marker
            v-if="
              circle.display[1] > currentZoom &&
                circle.display[0] <= currentZoom
            "
...
export default Vue.extend({
  ...
  methods: {
    ...
    prepData() {
      if (this.data) {
        this.data.forEach(d => {
          d["fill"] = d[this.variable]
            ? this.colorScale(d[`${this.variable}_numIncrease`])
            : "#CCC";
          d["r"] = this.radiusScale(d[this.variable]);
          d["currentDateFormatted"] = this.formatDate(d.date);
          d["numIncreaseFormatted"] = d[`${this.variable}_numIncrease`]
            ? d[`${this.variable}_numIncrease`].toLocaleString()
            : null;
          d["pctIncreaseFormatted"] = this.formatPercent(
            d[`${this.variable}_pctIncrease`]
          );
          d["totalNumFormatted"] = d[this.variable]
            ? d[this.variable].toLocaleString()
            : null;
    ...
    colorScale(d) {
      const scale = scaleSequential(interpolateYlGnBu)
        .domain([1, max(this.data, d => d[`${this.variable}_numIncrease`])])
        .clamp(false);
