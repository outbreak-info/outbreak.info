...
<div class="py-5">
  <p class="case-summary focustext">
    ...
    <span v-for="(country, idx) in mostCases" :key="idx">
      <router-link :data-tippy-info="`view ${country.locationName} over time`" :to="{
            name: 'Epidemiology',
            query: { location: country.locationName }
          }" class="hardest-hit text-sec font-weight-bold">{{ country.locationName }}</router-link>
  ...
  <p class="focustext">
    ...
    <span v-if="firstCases.length > 0">
      <router-link :data-tippy-info="`
        ${firstCases.map(d => d.locationName).sort().join(', ')}
      `" id="first-cases" :to="{ name: 'Epidemiology', query: { location: newCountries } }" class="text-sec font-weight-bold">{{ firstCases.length }} countries</router-link>
      have announced their <span class="text-highlight">first cases</span>, and
    </span>
    <router-link id="changing-countries" :data-tippy-info="`
      ${countriesAboveThreshold.map(d => d.locationName).sort().join(', ')}`" :to="{ name: 'Epidemiology', query: { location: changingCountries } }" class="text-sec font-weight-bold">{{ countriesAboveThresholdCount }} countries
    </router-link>
