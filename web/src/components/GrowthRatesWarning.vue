...
<script setup>
  ...
  const titleString = `No API data for ${locationCount.value} ${locationCount.value > 1 ? "locations" : "location"}`;
  ...
  const contentString = `${props.lineage} growth rates not available for ${locationNamesString}`;
