...
export default {
  ...
  methods: {
    ...
    drawPlot() {
      ...
      const lolliSelector = this.chart
        .selectAll('.lollipop')
            ...
            .call((update) => {
              if (this.animate) {
                update
                  ...
                  .attr(
                    'y1',
                    (d) => this.y(d.location_id) + this.y.bandwidth() / 2,
                  )
                  .attr(
                    'y2',
                    (d) => this.y(d.location_id) + this.y.bandwidth() / 2,
      ...
      const circleSelector = this.chart
        .selectAll('circle')
        .data(this.plottedData, (d) => d.location_id);
      ...
      yAxSelector.join(
        (enter) =>
          enter
            ...
            .attr(
              'class',
              (d) => `location-most-change ${d.location_id} y-axis`,
            )
            .attr('id', (d) => `location-change-${d._id}`)
            .attr('x', (d) => this.x(0))
            .attr('dx', (d) =>
              d.rightAlign ? this.radius * 1.5 : -1.5 * this.radius,
            )
            .attr('y', (d) => this.y(d.location_id) + this.y.bandwidth() / 2)
            .text((d) =>
              d.admin1
                ? `${trimText(
                    d.name.replace(' County', ''),
                    locationNameThresh,
                  )}`
      ...
      const annotationSelector = this.chart
