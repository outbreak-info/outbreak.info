  ...
  methods: {
    ...
    updateAxes() {
      ...
      this.x = scaleLinear()
        ...
        .domain([
          0,
          min([max(this.processedData.map((d) => d.proportion)) + 0.1, 1]),
        ]);
    ...
    tooltipOn(d) {
      ...
      ttip
        .select('#proportion')
        .html(`<b>${d.proportion_formatted}</b> ${this.mutationName}`);
      ttip
        .select('#counts')
        .text(
      ...
      selectAll(`#${d.pangolin_lineage.replace(/\./g, '_')}`).style(
        'opacity',
        1,
      );
    ...
    tooltipYAxisOn(value) {
      ...
      if (d.length === 1 && value !== 'other') {
        ...
        ttip
          .select('#proportion')
          .html(`<b>${d[0].proportion_formatted}</b> ${this.mutationName}`);
        ttip
          .select('#counts')
          .text(
    ...
    tooltipOff() {
      select(this.$refs.tooltip_by_lineage).style('display', 'none');
      ...
      textSelector.join(
        (enter) => {
          enter
            ...
            .attr(
              'y',
              (d) => this.y(d.pangolin_lineage) + this.y.bandwidth() / 2,
            )
            ...
            .style('text-anchor', (d) =>
              this.x(d.proportion) > textThresh ? 'end' : 'start',
            )
